...
function ApprovalLinkScreen() {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (attachment?.sourceHtml) {
          setPreviewHtml(attachment.sourceHtml);
        } else if (attachment?.dataBase64 && attachment.mimeType === DOCX_MIME) {
          try {
  ...
  const handleDecide = async (decision) => {
    ...
    try {
      ...
      if (decision === 'Approved') {
        try {
          const next = await getNextApprovalInBatch(request.batchId, request.sequence);
          if (next) {
            await sendApprovalEmail({
              toEmail: next.approverEmail,
              toName: next.approverName,
              fromName: request.requestedBy,
      ...
      setRequest((prev) => ({ ...prev, status: decision, comment }));
    } catch (err) {
  ...
  return (
    <div className="appr">
      <div className="appr__card">
        <span className="appr__brand">Esovia Group</span>
        <h1 className="appr__title">{request.agreementTitle || 'Document approval'}</h1>
        <p className="appr__subtitle">
          {request.requestedBy
            ? `${request.requestedBy} sent you this document for approval.`
            : 'You were sent this document for approval.'}
          {request.sequence > 1 && ' You are the next approver in this chain.'}
        </p>
        ...
        <div className="appr__doc">
          ...
          {previewHtml ? (
            ...
            !previewError && !request.attachment?.dataBase64 && (
...
export default ApprovalLinkScreen;
