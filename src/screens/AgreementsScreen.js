...
function AgreementsScreen() {
  ...
  const filtered = useMemo(() => {
    if (sideFilter === 'mine') {
      return agreements.filter((a) => a.createdBy === currentUser?.email);
    }
    return agreements;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      indexObject('agreement', created.id).catch((err) => console.warn('Background indexing failed:', err));
      handleCloseCreate();
  ...
  const statusClass = (status) => {
    switch ((status || '').toLowerCase()) {
      ...
      case 'generated': return 'agr__status--draft';
