// Shared between server/index.js (local dev proxy) and api/ask-ai.js
// (Vercel serverless function). This file is a THIN proxy — it just
// forwards the conversation + tool definitions to Claude and hands back
// the raw response. It NEVER touches Firestore itself: the actual tool
// execution (looking up accounts/agreements/documents) happens back on
// the CLIENT, using the same authenticated Firebase session and Firestore
// security rules as the rest of the app. That split means this server
// never needs its own Firebase Admin credentials — the only secret it
// holds is the Anthropic API key.
