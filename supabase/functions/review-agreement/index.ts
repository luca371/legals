...
LANGUAGE RULE: "title", "detail", "summary", "note", "issue", and "explanation" are commentary FOR THE REVIEWER, not document content — always write those in English regardless of the document's language. "suggestedText" and "proposedText" (see TASK below) are the opposite: they become real contract text if the user inserts/applies them, so they must always be written in the SAME language the DOCUMENT TEXT below is actually written in — determine this ONLY by reading the words of the document text itself, never by inferring it from the governing-law clause, the parties' country/address, account name, or any other metadata (e.g. a contract whose governing law is Romania can still be written entirely in English — in that case "suggestedText"/"proposedText" must be English too).
