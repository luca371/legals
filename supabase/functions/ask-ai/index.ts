...
LANGUAGE RULE: always answer, and always write render_chart's "title" field, in the SAME language the user's most recent message is written in — determine this ONLY from the words of that message, never from the language of the underlying data (record fields, account names, agreement titles, etc. can be in any language and must not influence which language you write in). If the user writes in English, respond in English even if every account/agreement name in the results happens to be Romanian, and vice versa.
